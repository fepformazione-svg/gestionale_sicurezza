...
class _ImpresePageState extends State<ImpresePage> {
  ...
  Future<void> apriDialogNuovaImpresa() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Impresa salvata nel database'),
      ),
    );
  }
      ...
      child: Column(
        ...
        children: [
          const PageHeader(
            title: 'Imprese',
            subtitle:
                'Archivio aziende, clienti e anagrafiche operative.',
          ),
                                              ...
                                              Expanded(
                                                child:
                                                  ...
                                                  children: [
                                                    ...
                                                    Text(
                                                      [
                                                        if ((item.partitaIva ?? '').isNotEmpty)
                                                          'P.IVA: ${item.partitaIva}',
                                                        if ((item.codiceFiscale ?? '').isNotEmpty)
  ...
  final ragioneSocialeController =
      TextEditingController(text: impresa.intestazione);
  final partitaIvaController =
      TextEditingController(text: impresa.partitaIva ?? '');
  final codiceFiscaleController =
      TextEditingController(text: impresa.codiceFiscale ?? '');
  final indirizzoController =
      TextEditingController(text: impresa.indirizzo ?? '');
  final telefonoController =
      TextEditingController(text: impresa.telefono ?? '');
  final referenteController =
      TextEditingController(text: impresa.referente ?? '');
  ...
  ScaffoldMessenger.of(context).showSnackBar(
    const SnackBar(
      content: Text('Impresa modificata correttamente'),
    ),
  );
}
}
