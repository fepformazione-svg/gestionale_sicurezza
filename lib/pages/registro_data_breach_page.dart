...
class _RegistroDataBreachPageState extends State<RegistroDataBreachPage> {
  ...
  Future<Uint8List> generaPdfSingoloDataBreachBytes(DataBreach elemento) async {
    String valore(String testo) {
      final pulito = testo.trim();
      return pulito.isEmpty ? '-' : pulito;
    }
    ...
    pw.Widget sezione(String titolo) {
      return pw.Padding(
    ...
    pw.Widget riga(String etichetta, String contenuto) {
      return pw.Padding(
      ...
      pw.MultiPage(
        ...
        header: (context) => pw.Column(
          ...
          children: [
            ...
            pw.Text(
              'Dettaglio evento - GDPR 679/2016',
              style: const pw.TextStyle(fontSize: 10),
            ),
            pw.Divider(),
          ],
  ...
  Future<void> mostraAnteprimaPdfSingoloDataBreach(DataBreach elemento) async {
    final bytes = await generaPdfSingoloDataBreachBytes(elemento);
    ...
    await showDialog<void>(
      context: context,
      builder: (_) => AlertDialog(
        title: const Text('Anteprima PDF Data Breach'),
        contentPadding: const EdgeInsets.fromLTRB(16, 12, 16, 0),
        content: SizedBox(
          width: larghezzaDialog,
          height: altezzaDialog,
        ...
        actions: [
          OutlinedButton.icon(
            onPressed: () async {
              ...
              await mostraAnteprimaPdfSingoloDataBreach(elemento);
            },
            icon: const Icon(Icons.picture_as_pdf_outlined),
            label: const Text('PDF singolo'),
          ),
          TextButton(
            onPressed: () => Navigator.pop(context),
            child: const Text('Chiudi'),
